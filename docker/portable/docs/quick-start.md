...
If you came through [utility-support](https://github.com/ryokubaka/utility-support), skip this list. That script already created the owner, turned signups off, built one org with IT and OT groups, and turned on account recovery. Log in with `VW_OWNER_EMAIL` / `VW_OWNER_PASSWORD`. Then send a test email from `/admin`.
...
6. After the org exists, turn on account recovery. See [organizations.md](organizations.md).
